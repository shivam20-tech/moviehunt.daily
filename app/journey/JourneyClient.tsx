...
export default function JourneyClient({ hunts: HUNTS_DATA }: JourneyClientProps) {
  ...
  const FIRST_DAY = HUNTS_DATA.length > 0 ? Math.min(...HUNTS_DATA.map((h) => h.day)) : 1;
  const LATEST_DAY = HUNTS_DATA.length > 0 ? Math.max(...HUNTS_DATA.map((h) => h.day)) : 1;
  const BASE_MILESTONES = [1, 10, 25, 50, 75];
  const MILESTONE_DAYS = new Set([...BASE_MILESTONES, LATEST_DAY].filter((d) => d <= LATEST_DAY));
    ...
    <main className="min-h-screen bg-[#0a0a0f] text-[#f4f4f0] selection:bg-[#e5a93c] selection:text-[#0a0a0f]">
      ...
      <section className="py-10 sm:py-16 max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        ...
        {filteredHunts.length === 0 ? (
          ...
          <MilestoneLayout hunts={filteredHunts} milestoneDays={MILESTONE_DAYS} latestDay={LATEST_DAY} />
        ) : (
...
function MilestoneLayout({
  ...
  const segments: Array<{ milestoneDay: number | null; cards: HuntItem[] }> = [];
  let currentSegment: HuntItem[] = [];
  let currentMilestone: number | null = null;

  for (const hunt of hunts) {
    if (milestoneDays.has(hunt.day) && !segments.some((s) => s.milestoneDay === hunt.day)) {
      ...
      {segments.map((seg, i) => (
        <div key={i}>
          {seg.milestoneDay !== null && (
            <div className="flex items-center gap-4 mb-6" aria-label={`Day ${seg.milestoneDay} milestone marker`}>
...
// ── Flat grid ─────────────────────────────────────────────────────────────────
function FlatGrid({ hunts }: { hunts: HuntItem[] }) {
  return <HuntCardGrid cards={hunts} />;
}

// ── Shared card grid ──────────────────────────────────────────────────────────
function HuntCardGrid({ cards }: { cards: HuntItem[] }) {
  return (
